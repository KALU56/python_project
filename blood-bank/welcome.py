...
class WelcomePage:
    ...
    def go_to_supervisor(self):
        ...
        superviser_button = tk.Button(superviser_window, text="superviser dash bord", font=("Helvetica", 12), 
                                        command=lambda: superviser(tk.Toplevel(superviser_window)))
        registration_button.pack(pady=10)
...
class superviserLoginForm:
    def __init__(self, root):
        self.root = root
        self.root.title("superviser Login")
        self.root.geometry("400x300")
    ...
    def login(self):
        ...
        if not username or not password:
            ...
            if username in data and password in data:
                donor_activity_window = tk.Toplevel(self.root)
                donor_activity_window.title("superviser Activity")
                donor_activity_window.geometry("400x400")

                seedoner_button = tk.Button(superviser_activity_window, text="see doners", font=("Helvetica", 12), 
                                                command=lambda: seedoners(tk.Toplevel(donor_activity_window)))
                seedoner_button.pack(pady=10)

                send_medical_history_button = tk.Button(donor_activity_window, text="send Medical History", font=("Helvetica", 12), 
                                                   command=self.go_to_medical_history)
                send_medical_history_button.pack(pady=10)


                donor_exit_button = tk.Button(donor_activity_window, text="Exit", font=("Helvetica", 12), command=superviser_activity_window.destroy)
                donor_exit_button.pack(pady=10)

            else:
                messagebox.showerror("Error", "Invalid username or password.")
class donor_activity_window:
